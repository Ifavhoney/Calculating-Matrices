...
//2.3 A program to implement the BoyerMoore algorithm. (30%)
...
//Attempted for part marks
Results BoyerMoore(char *test, char *prompt){
     int size = (int)strlen(prompt)-1;

       Map *map = badTable(prompt);
  //  Map *map2 = goodTable(prompt);
    ...
    char *prompt = calloc(1, 100);
       ...
       //Finds total matches in the given array
        ...
        //Find matches per line
            Results j = BoyerMoore(array[i], prompt);
